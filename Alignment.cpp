...
void AlignmentAlgorithm::buildStates() {
    ...
    for (size_t i = 0; i < L; ++i) {
        ...
        // seq1 状态
        if (c1 == '-')                          seq1_state_[i] = GAP;
        else if (c1 == c2)                      seq1_state_[i] = MATCH;
        else                                     seq1_state_[i] = FAIL;
        // seq2 状态
        if (c2 == '-')                          seq2_state_[i] = GAP;
        else if (c2 == c1)                      seq2_state_[i] = MATCH;
        else                                     seq2_state_[i] = FAIL;
    }
}
...
void Hirschberg::initMatrix() {
    // 同 Needleman–Wunsch 完整填表，方便 getMatrix()
    for (int i = 1; i <= m_; ++i) M_[i][0] = i * gap_open_;
    for (int j = 1; j <= n_; ++j) M_[0][j] = j * gap_open_;
    for (int i = 1; i <= m_; ++i) {
        for (int j = 1; j <= n_; ++j) {
